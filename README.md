...
Publication diagnostics retain the positive direction under influence screening, conditional logistic regression, continuous loss share, leave-one-year-out analysis, and 25/50/100-km spatial clustering. A post-registration eligibility check also retained positive confidence intervals when the minimum pre-index natural-forest threshold was varied from 50% to 80% (**+4.23 to +5.89 points**), reducing concern that the sign is unique to the registered 70% cutoff. It does not remove the two material constraints: **41.4%** of temporally eligible matched sets failed complete forest/observation support, and the pre-exposure negative control was positive (**+2.31 points**, 95% CI **+1.13 to +3.50**). That baseline difference indicates pre-existing land-change trajectory or residual confounding and rules out a causal reading of the main estimate. Four unique secondary threshold/follow-up checks now report Holm-adjusted p-values as a family; the single registered primary test retains its raw p-value.
...
- a validated, sensor-stratified SiPongi portal-record archive for July-November 2015-2023 and 2025-2026 partial context. SiPongi 2024 is explicitly shown as unavailable—not zero—because six responses failed geography validation and were quarantined; this does not remove the separate NASA FIRMS completed-2024 country layer.
...
The maintainable Next.js App Router version is in [apps/evidence-explorer](apps/evidence-explorer). It uses CesiumJS for a real WGS84 WebGL globe: a local NASA Blue Marble Earth surface plus frozen geoBoundaries ADM1 polygons are the actual clickable/hoverable geometry. The SiPongi current-five and GWIS legacy-four systems remain separate; legacy Kalimantan Timur is one documented topological union of current East and North Kalimantan, not two GWIS values. Run `npm install`, `npm run sync-globe-assets`, then `npm run dev` in that folder for local development; `npm run build` creates the production Next.js application. The build-time synchronizer accepts only the canonical aggregate bundle and refuses raw SiPongi records, sensitive location fields, and quarantined responses. The globe does not use Cesium Ion, live tile services, raw hotspot points, or a risk layer. An optional explanation layer is restricted to compact public evidence packs and validates citations and numeric claims before display; it has no retrieval access to the raw research archive. Its 24 displayed suggestions are answered locally from fixed, tested fact-ID contracts. Free-form questions use server-side Kimi K2.5, require a versioned privacy acknowledgement in both UI and API, and are covered by the public `/privacy` notice explaining Moonshot processing.
