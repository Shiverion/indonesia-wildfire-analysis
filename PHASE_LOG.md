...
| Public dashboard deployment | **Ready** | The coordinate-free Next.js static export is deployed at `https://fire-research.shiverion.com`; the production response returned HTTP 200. The deployment excludes raw research archives and private coordinates. Receipt: `outputs/quality/vercel_deployment.json`. |
...
## Execution update -- publication safeguards and suggestion-answer audit (30 August 2026)

| Workstream | Status | Evidence / decision |
|---|---|---|
| Selection-bias sensitivity | **Completed; sign stable, transportability still limited** | A post-registration diagnostic varied the minimum pre-index natural-forest eligibility threshold across 50%, 60%, 70%, and 80%. The retained sample ranged from 10,774 to 3,861 matched sets; adjusted differences ranged from +5.50, +5.08, +5.89, to +4.23 percentage points, and every 95% interval remained above zero. This reduces concern that the direction is unique to the 70% cutoff but does not establish missing-at-random exclusion or remove the 41.4% attrition boundary. |
| Secondary-test multiplicity | **Corrected transparently** | The single registered primary test retains its raw p-value. The four unique secondary threshold/follow-up checks now report both raw and Holm-adjusted p-values under a dated post-registration reporting amendment. No estimate, model, eligibility rule, or outcome definition changed. |
| SiPongi 2024 gap | **Prominent and nonzero-safe** | The public map/report now states that 2024 is unavailable because six SiPongi responses failed geography validation and were quarantined. The annual chart renders a visible gap instead of bridging 2023 to 2025 or substituting zero; the separate NASA FIRMS completed-2024 country layer is explicitly distinguished. |
| Legacy/current Kalimantan geography | **Build-enforced** | The globe-asset verifier requires exactly four legacy GWIS units, forbids a standalone Kalimantan Utara feature in that system, and separately requires exactly five current SiPongi provinces. The legacy Kalimantan Timur topological-union check remains active. |
| Chat privacy boundary | **Enforced at UI and API** | Free-form questions require the exact current privacy acknowledgement; missing and stale versions returned HTTP 428 in live route tests. The privacy page names the bounded content sent to Moonshot and warns about external retention/model-use terms. Raw questions are not retained by this application. |
| Suggested-question quality | **24/24 passed live route audit** | Every displayed suggestion now has a nonempty fact-ID contract. All 24 were sent through the live local API and returned a nonempty answer, one or more known citations, and `validator=passed`. Suggested answers are generated locally from the current contracted facts, so they do not depend on Kimi availability, consume model quota, or send visitor content to Moonshot. Receipt: `outputs/quality/assistant_suggestion_audit.json`. |
| Free-form assistant resilience | **Fail-closed with bounded retry** | Kimi remains available only for free-form explanation after consent. Transient 429/500/502/503/504 responses receive at most three bounded attempts; malformed or ungrounded output is never displayed and becomes the documented insufficient-evidence response. |
