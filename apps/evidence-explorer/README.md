...
The report also includes an accountable explanation layer. Its 24 displayed suggestions are locally curated: every suggestion has a fixed fact-ID contract and returns the current statements from that contract without calling an external model. Free-form questions use server-side Kimi with one compact public evidence pack. It is deliberately not a general chatbot: answers require fact-ID citations, and the server rejects unknown citations or numbers before they reach the browser. Raw questions are not stored by the application and Kimi reasoning content is never exposed.
...
- Each displayed answer includes a receipt with the answer source/model, prompt, corpus, and privacy-notice versions, cited evidence IDs, validator result, latency, and a statement that this application did not retain the raw question. Curated suggestions are answered locally and their receipt records that no external processor was called. Before a free-form question is sent, the UI requires an explicit acknowledgement that the question, up to four recent messages, and the selected public evidence pack are processed by Moonshot's API and remain subject to the provider's retention and model-use terms. The API enforces the same notice version and rejects missing or stale acknowledgement with HTTP 428.
